...
const updateProduct = async (req, res) => {
  try {
    ...
    if (quantidade === 0) {
      res.status(400);
      return res.json({ erro: 'Informe um numero maior ou menor que 0' });
    }

    if (quantidade < 0) {
      if (Math.abs(quantidade) > cartProduct.quantidade) {
        res.status(400);
        return res.json({ erro: 'Não tem como remover mais items do que já existe' });
      }
...
module.exports = {
  showCart, cleanCart, addProductsToCart, removeProduct, updateProduct,
};
